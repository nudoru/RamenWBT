			...
			if (lastMouseDown != _mouseIsDown) {
				...
				// mouse is up
					...
					if (!disabledEvents[MouseEvent.CLICK] && currentTarget == lastDownTarget) {
						...
						// Customization by Matt Perkins
						// With accessibility pressing enter resulted in a double click, one from Flash and one from here
						if (!(currentTarget is MovieClip)) {
							currentTarget.dispatchEvent(_lastEvent);
							dispatchEvent(_lastEvent);
						}
