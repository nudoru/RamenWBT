...
package ramen.player {
	...
	public class  PopUpManager extends Sprite {
		...
		public static const MAXSCROLLWINDOWHEIGHT	:int = 450;
		...
		public function createTextScrollingPopUp(d:PopUpDataObject):* {
			...
			// test to see if it was successful
			if (window) {
				setWindowText(window, d, true);
			} else {
				trace("createTextScrollingPopUp - couldn't find type");
				return undefined;
			}
			if (window.body_txt.height > MAXSCROLLWINDOWHEIGHT) {
				window.body_txt.autoSize = TextFieldAutoSize.NONE;
				window.body_txt.height = MAXSCROLLWINDOWHEIGHT;
				window.scrollbar_sb.y = window.body_txt.y;
				window.scrollbar_sb.scrollTarget = window.body_txt;
				window.scrollbar_sb.height = MAXSCROLLWINDOWHEIGHT;
				window.scrollbar_sb.update();
				//window.scrollbar_sb.focusManager.deactivate();
			} else {
				window.scrollbar_sb.visible = false;
			}
			setupSize(window, d.popuptype, (d.buttons.length > 0));
			return window;
		}
		...
		private function setupSize(w:Object, t:String, b:Boolean):void {
			...
			var buttonAreaHight:int;
			try {
				buttonAreaHight = w.buttonbg_mc.height;
			} catch (e:*) {
				buttonAreaHight = 40;
			}
			if (w.title_txt) {
		...
		private function setupButtons(d:PopUpDataObject):void {
			...
			var buttonAreaHight:int;
			try {
				buttonAreaHight = d.window.buttonbg_mc.height;
			} catch (e:*) {
				buttonAreaHight = 40;
			}
			switch (d.popuptype) {
				...
				case PopUpType.TEXT_SCOLLING:
					...
					if (d.buttons.length) {
						for (var i:int = 0; i < d.buttons.length; i++) {
							...
							if (d.buttons.length == 1) {
								button.x = (d.window.bg_mc.width/2)-(button.width/2);
							}
							button.y = textBottom + ((buttonAreaHight / 2) - (button.height / 2));//(DEFAULT_BORDER/2)-2
