	...
	public class ListTermCard extends ListTemplate {
		...
		override protected function setItem(l:int, c:int):void {
			...
			if (TheCards[c].selected) {
				...
				TweenMax.to(card, 1, { z:getOrigionalZ(c), localRotationY:-180, ease:Back.easeOut, onComplete:handleCardHide, onCompleteParams:[c] } );
				BMUtils.applyDropShadowFilter(cardLayer, 5, 45, 0x000000, .3, 5, 1);
			} else {
				//trace("show");
				TheCards[c].selected = true;
				TweenMax.to(card, .5, { z:getHighZ(), localRotationY:0, ease:Quadratic.easeOut, onComplete:handleCardShow, onCompleteParams:[c] } );
