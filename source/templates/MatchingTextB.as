...
[ ] choice dropped on another choice that's on a target - both stay up on the target
...
package {
	...
	public class MatchingTextB extends QuestionTemplate {

		protected var HiChoiceIdx		:int;
		protected var Matches			:Array;
		
		public function MatchingTextB():void {
			super();
		...
		protected function setChoiceToCorrectMatch(q:int, c:int, a:Boolean = true):void {
			...
			var x:int = mi.x + mi.bg_mc.x+5;
			var y:int = mi.y + mi.bg_mc.y+5;
		...
		override protected function renderChoices():void {
			...
			if (!choiceColWidth) choiceColWidth = _PageRenderer.pageColumnWidthThree;
			...
			for (var k:int = 0; k < len; k++) {
				...
				m.text_txt.htmlText = _Questions[_CurrentQuestion].getChoiceMatch(k);
				...
				m.textbg_mc.x = matchTextX;
				m.textbg_mc.scaleX = (m.text_txt.width + 20) * .01;
				m.textbg_mc.scaleY = (10+m.text_txt.height) * .01;
				
				m.matchbg_mc.scaleX = (4 + (m.textbg_mc.x - matchColWidth)+(m.bg_mc.scaleX*100) + (m.textbg_mc.scaleX*100))*.01;
				m.matchbg_mc.scaleY = (14+m.text_txt.height) * .01;
				...
				mY += matchColYSpc + (m.matchbg_mc.scaleY*100);
			}
			
			var cArray:Array = _Questions[_CurrentQuestion].getRandomizedChoiceIndexesArray();
			...
			for (var i:int = 0; i < len; i++) {
				...
				c.matchIdx = _Questions[_CurrentQuestion].getMatchIndexForChoice(cArray[i]);
				c.displayIdx = i;
				...
				if (_Animate) {
					c.alpha = 0;
					c.y -= 50;
					TweenLite.to(c, 1, { alpha:1, y:cY, delay:(i*.5), ease:Quadratic.easeOut, onComplete:enableChoice, onCompleteParams:[_CurrentQuestion, cArray[i]] } );
				} else {
